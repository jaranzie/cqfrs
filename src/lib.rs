...
//             quotient_bits,
//             bits_per_slot,
//             std::mem::size_of::<Block<Remainder>>() as u64,
//             invertable,
//         );
...
//             buffer.offset(std::mem::size_of::<Metadata>() as isize) as *mut Block<Remainder>
//         };
...
//                 return Ok(());
//             } else if !self.blocks.has_metadata_bits_set(quotient + 1) {
//                 let count_quotient = quotient + 1;
//                 count.to_ne_bytes();
//                 self.blocks.set_slot(count_quotient, count - 1);
//             }
//             if !self.blocks.has_metadata_bits_set(quotient + 1) {}

//             block.flip_runend(slot_index);
//             self.metadata
//                 .num_occupied_slots
...
impl Metadata {
    fn new(
        ...
        let num_slots: u64 = 1 << lognslots;
        let real_num_slots: u64 = (num_slots as f64 + 10 as f64 * (num_slots as f64).sqrt()) as u64;
        let num_blocks = (real_num_slots + SLOTS_PER_BLOCK as u64 - 1) / SLOTS_PER_BLOCK as u64;
